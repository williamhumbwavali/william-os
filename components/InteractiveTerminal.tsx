...
const BOOT_SCRIPT: BootLine[] = [
  ...
  { prompt: false, text: "> Engenheiro de Software · Luanda, Angola", tone: "muted" },
  { prompt: true, text: "cat focus.txt" },
  {
    prompt: false,
    text: "Construo frameworks, produtos e negócios — do zero ao ar.",
    tone: "muted",
  },
  { prompt: true, text: "ls ./projects" },
  { prompt: false, text: "lithe-php/ bando/  baza/  rialse/  bvf/", tone: "cyan" },
];
...
export default function InteractiveTerminal({
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const line of BOOT_SCRIPT) {
        setHistory((h) => [
          ...h,
          { kind: line.prompt ? "input" : "output", text: "", tone: line.tone ?? "ink" },
        ]);
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "ArrowUp") {
      ...
      const nextPointer =
        historyPointer === null ? cmdHistory.length - 1 : Math.max(0, historyPointer - 1);
  ...
  return (
    <div className="rounded-lg border border-line bg-panel overflow-hidden">
      <div className="flex items-center gap-2 border-b border-line px-4 py-2.5 bg-panelAlt">
        ...
        <span className="ml-3 font-mono text-xs text-muted">~/william — zsh</span>
      </div>
      ...
      >
        {history.map((line, i) =>
          line.kind === "input" ? (
            ...
            <div key={i} className={`whitespace-pre-wrap pl-6 ${toneClass(line.tone)}`}>
              {line.text}
            </div>
          )
        )}
        ...
        {booting && <span className="ml-6 animate-blink text-ink">▍</span>}
      </div>
