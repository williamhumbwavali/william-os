...
export default function Sidebar({
  ...
  const groups: { label: string; ids: string[] }[] = [
    ...
    {
      label: "projects",
      ids: ["baza", "bvf", "lithe", "bando", "rialse"],
    },
    {
      label: "skills",
      ids: ["skills"],
  ...
  const rootFile = files.find((file) => file.id === "contact");

  if (!rootFile) return null;

  return (
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-line bg-panelAlt lg:flex">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-4 font-mono text-[13px]">
              ...
              {group.ids.map((id) => {
                ...
                return (
                  <button
                    ...
                    className={`focus-ring flex w-full items-center gap-2 rounded px-2 py-1.5 text-left transition-colors ${
                      active
                        ? "bg-line/60 text-ink"
                  ...
                  >
                    <DotIcon
                      colorClass={active ? "bg-cyan" : "bg-mutedDark"}
                    />
        ...
        <div className="mt-2 border-t border-lineSoft pt-2">
          ...
          >
            <DotIcon
              colorClass={
                activeId === "contact" ? "bg-cyan" : "bg-mutedDark"
              }
            />

            <span
              className={
                activeId === "contact" ? extColor[rootFile.ext] : ""
              }
            >
              {rootFile.label}
            </span>
